...
class TestCdtool(unittest.TestCase):
    """Tests for `cdtool` package."""
    ...
    def test_parse_arguments(self):
        ...
        res = cdtoolcmd._parse_arguments('hi', [])
        ...
        res = cdtoolcmd._parse_arguments('hi', someargs)
...
format=%(asctime)s %(name)-12s %(levelname)-8s %(message)s""")
            ...
            res = cdtoolcmd._parse_arguments('hi', ['--logconf',
                                                                       logfile])
            cdtoolcmd._setup_logging(res)
