...
def main(args):
    ...
    :return: return value of :py:meth:`cdtool.runner.CdtoolRunner.run`
    ...
    Invokes run() method on CdtoolRunner

    """.format(version=cdtool.__version__)
    theargs = _parse_arguments(desc, args[1:])
    theargs.program = args[0]
    theargs.version = cdtool.__version__

    try:
        _setup_logging(theargs)
        return CdtoolRunner(theargs.exitcode).run()
